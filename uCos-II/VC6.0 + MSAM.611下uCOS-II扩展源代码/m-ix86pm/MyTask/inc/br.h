...
typedef struct {
	...
	char             ifname[64]; /* Make life easier for brcfg */
        struct nic *dev;	

	/* Because we dont use malloc */
	struct fdb fdb[256];	/* head of per port fdb chain */
...
extern int br_recv(struct nic *d, void *pkt, int len);
/* externs */
extern void br_register_port(struct nic *dev);
